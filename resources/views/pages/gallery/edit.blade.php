    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form action="/gallery/{{ $gallery->id }}/edit" method="post" enctype="multipart/form-data">
            ...
            <label class="block mt-4 text-sm">
                ...
                <select
                    ...
                    @foreach ($products as $product)
                        @if (old('product_id', $gallery->product_id) === $product->id)
            ...
            <label class="block mt-4 text-sm">
                ...
                <input type="hidden" name="oldImage" value="{{ $gallery->url }}">
                <div class="relative hidden mr-3 mt-1 rounded md:block" style="height: 74px; width: 74px">
                    <img class="object-cover w-full h-full rounded" src="{{ asset('storage/' . $gallery->url) }}"
                        alt="" loading="lazy" />
